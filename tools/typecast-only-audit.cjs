...
assert(guide.includes('emitTimedSpeech:') && guide.includes('return NO;'), 'SDK must forward timed text without playing its own speech');
assert(bridge.includes('spokenSafety') && bridge.includes('spokenGuide'), 'Timed speech must retain the app voice route');
assert(bridge.includes('isSpeechTargetAhead:') && bridge.includes('validUntil'), 'Navigation must retain target/deadline checks');
