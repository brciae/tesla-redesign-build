// Adapted from Supertone Inc. Supertonic (MIT). See VoiceEngineLicense.txt.
// Modified: bounded duration/steps, cooperative cancellation, CPU thread limit.
import Foundation
import Accelerate
...
class TextToSpeech {
    ...
    private func _infer(_ textList: [String], _ langList: [String], _ style: Style, _ totalStep: Int, speed: Float = 1.05) throws -> (wav: [Float], duration: [Float]) {
        try checkCancelled()
        guard textList.count == 1, (1...16).contains(totalStep), speed.isFinite, (0.7...1.5).contains(speed) else { throw NSError(domain: "TTS", code: 2) }
        ...
        guard duration.count == bsz, duration.allSatisfy({ $0.isFinite && $0 > 0 && $0 <= 35 }) else { throw NSError(domain: "TTS", code: 3, userInfo: [NSLocalizedDescriptionKey: "음성 길이 처리 오류"]) }
        try checkCancelled()
...
func loadProfileStyle(_ profile: VoiceProfile, folder: URL) throws -> Style {
    guard VoicePackManifest.voices.contains(profile.base), VoicePackManifest.voices.contains(profile.partner) else { throw NSError(domain: "VoiceStyle", code: 1) }
    if profile.partner == profile.base || (profile.blend >= 1 && profile.rhythm >= 1) {
        return try loadVoiceStyle([folder.appendingPathComponent("voice_styles/" + profile.base + ".json").path], verbose: false)
    }
    func read(_ name: String) throws -> VoiceStyleData {
        try JSONDecoder().decode(VoiceStyleData.self, from: Data(contentsOf: folder.appendingPathComponent("voice_styles/" + name + ".json")))
    }
    let a = try read(profile.base), b = try read(profile.partner)
    func blend(_ x: VoiceStyleData.StyleComponent, _ y: VoiceStyleData.StyleComponent, _ weight: Float) throws -> ORTValue {
        guard weight.isFinite, (0...1).contains(weight), x.dims == y.dims, x.dims.count == 3,
              x.dims[0] == 1, x.dims.allSatisfy({ $0 > 0 && $0 <= 4096 }) else { throw NSError(domain: "VoiceStyle", code: 2) }
        let expected = x.dims.reduce(1, *)
        func flatten(_ value: VoiceStyleData.StyleComponent) throws -> [Float] {
            guard value.data.count == 1, value.data.allSatisfy({ $0.count == value.dims[1] && $0.allSatisfy({ $0.count == value.dims[2] }) }) else { throw NSError(domain: "VoiceStyle", code: 3) }
            let result = value.data.flatMap { $0.flatMap { $0 } }
            guard result.count == expected, result.allSatisfy({ $0.isFinite && abs($0) < 10000 }) else { throw NSError(domain: "VoiceStyle", code: 4) }
            return result
        }
        let left = try flatten(x), right = try flatten(y)
        var values = zip(left, right).map { $0.0 * weight + $0.1 * (1 - weight) }
        guard values.allSatisfy({ $0.isFinite }) else { throw NSError(domain: "VoiceStyle", code: 5) }
        return try ORTValue(tensorData: NSMutableData(bytes: &values, length: values.count * MemoryLayout<Float>.size), elementType: .float, shape: x.dims.map { NSNumber(value: $0) })
    }
    return try Style(ttl: blend(a.style_ttl, b.style_ttl, profile.blend), dp: blend(a.style_dp, b.style_dp, profile.rhythm))
}
